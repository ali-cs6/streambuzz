  ...
  const user = await User.aggregate([
    ...
    },
    {
      $lookup: {
        ...
        pipeline: [
          {
            // since videos populated to owners
            // we fetch it from users to videos(owner field)
            $lookup: {
              from: "users",
              localField: "owner",
              foreignField: "_id",
              ...
              pipeline: [
                {
                  //include only these in the owner field
                  $project: {
                    fullname: 1,
                    username: 1,
                    avatar: 1,
